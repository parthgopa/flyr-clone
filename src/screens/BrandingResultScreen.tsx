...
const IMAGE_HEIGHT = Math.min(SCREEN_WIDTH * 1.1, SCREEN_HEIGHT * 0.55);
...
export default function BrandingResultScreen({ route, navigation }: any) {
    ...
    useEffect(() => {
        ...
        const interval = setInterval(async () => {
            try {
                ...
                if (data.images.length > images.length) setImages(data.images);
                ...
                console.error("Branding poll error:", err);
            }
        }, POLL_INTERVAL);
    ...
    const onScroll = useCallback(
        (e: NativeSyntheticEvent<NativeScrollEvent>) => {
            ...
            if (index >= 0 && index < allImages.length) setActiveIndex(index);
        },
        [allImages.length]
    );
    ...
    const showDownloadAllDialog = () => {
        Alert.alert(
            "Download All Images",
            `Download all ${images.length} branded images to your gallery?`,
            [
                { text: "Cancel", style: "cancel" },
                { text: "Download All", onPress: handleDownloadAll },
            ]
        );
    };
            ...
            <View style={styles.statusBar}>
                {!isDone ? (
                    ...
                    <View style={styles.doneRow}>
                        ...
                        <AppText style={styles.doneText}>All branded images generated!</AppText>
                    </View>
                )}
            </View>
            ...
            {isDownloadingAll && (
                <View style={styles.progressModal}>
                    <View style={styles.progressModalContent}>
                        ...
                        <AppText style={styles.progressModalTitle}>Downloading Images...</AppText>
...
const styles = StyleSheet.create({
    ...
    scenarioItemDone: { opacity: 0.7 },
    scenarioItemActive: { opacity: 1 },
    scenarioDot: {
        width: 16,
        height: 16,
        borderRadius: 8,
    ...
    scenarioLabelDone: { color: theme.colors.success },
    scenarioLabelActive: { color: theme.colors.accent, fontWeight: "600" },
    // Footer
    footer: {
        paddingHorizontal: theme.spacing.screenPadding,
